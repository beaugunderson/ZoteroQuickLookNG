Zotero.ZoteroQuickLookQuickFormat = {
  init: function () {
    ...
    // Zotero Quick Look takes priority in event listeners, so we need to remove
    // and readd the original.
    var originalListener = Zotero_QuickFormat._onQuickSearchKeyPress;

    document
      .getElementById("quick-format-entry")
      .addEventListener("keypress", this.onKey, true);

    // Override ESC key for the panel to prevent it closing if quick look is
    // open. In this case the first esc press should close the quicklook and
    // second should close the panel
    document
      .getElementById("quick-format-reference-list")
      .addEventListener("keypress", this.captureEsc, true);
  ...
  onKey: function (event) {
    ...
    Zotero.debug(
      "ZoteroQuickLook: Got key code " + event.keyCode + " char is " + key
    );
    ...
    if (
      ...
      Zotero.debug(
        "ZoteroQuickLook: Got key cmd+y, up arrow or down arrow. Start looking for item."
      );

      var referenceBox = document.getElementById("quick-format-reference-list");

      if (!referenceBox.hasChildNodes() || !referenceBox.selectedItem) {
        return;
      }

      // This is an ugly workaround to capture Esc. If quick look is open, we
      // do not want to close on Esc
      var items = Zotero.Items.get([
        referenceBox.selectedItem.getAttribute("zotero-item"),
      ]);
